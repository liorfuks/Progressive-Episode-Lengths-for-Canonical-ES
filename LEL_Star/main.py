...
def main(ep_per_cpu, game, configuration_file, run_name):
    ...
    # Set step limitation
    double_after = [20, 60, 140, 300, 620]
    d_t = 0
    max_step = np.zeros(1)
    if rank == 0:
       max_step[0] = policy.max_episode_len
    comm.Bcast([max_step, MPI.INT], root=0)
    policy.max_episode_len = int(max_step[0])
    ...
    if rank == 0:
        ...
        logger.log('Max step'.ljust(25) + '%d' % policy.max_episode_len)
    ...
    while True:
        ...
        # LEL doubling
        if ((time.time()-start_time)/60)>double_after[d_t]:
            policy.double_episode_length()
            d_t += 1
